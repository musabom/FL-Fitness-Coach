...
router.get("/dashboard/today", async (req, res): Promise<void> => {
  ...
  res.json({ date: dateStr, nutrition, training });
});
...
router.get("/dashboard/weekly", async (req, res): Promise<void> => {
  ...
  // Build per-day maps
  const nutByDate = new Map<string, { calories: number; protein_g: number; carbs_g: number; fat_g: number }>();
  ...
  res.json({ week_start: weekStart, week_end: weekEnd, totals, days });
});
