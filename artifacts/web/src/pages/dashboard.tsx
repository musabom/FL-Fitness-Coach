...
// ── Coach Message Card (member ↔ coach chat) ──────────────────────────────────
interface CoachMessage { id: number; content: string; from_coach: boolean; read_at: string | null; created_at: string; }
...
function CoachMessageCard() {
  ...
  // Always poll — even when card is closed — so badge stays live
  const messagesQuery = useQuery<CoachMessage[]>({
    queryKey: ["messages", "me"],
    queryFn: () => customFetch("/api/messages/me"),
    refetchInterval: 20_000,
    refetchOnWindowFocus: true,
  });

  const send = useMutation({
    mutationFn: (content: string) => customFetch("/api/messages/me", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ content }),
    }),
    onSuccess: () => { setText(""); qc.invalidateQueries({ queryKey: ["messages", "me"] }); },
    onError: () => toast({ title: "Failed to send", variant: "destructive" }),
  });

  const markRead = useMutation({
    mutationFn: () => customFetch("/api/messages/me/read", { method: "POST" }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["messages", "me"] }),
  });

  const messages = messagesQuery.data ?? [];
  const unread = messages.filter(m => m.from_coach && !m.read_at).length;

  // Toast notification when new unread messages arrive while card is closed
  useEffect(() => {
    if (!open && unread > prevUnreadRef.current && prevUnreadRef.current !== -1) {
      toast({
        title: "💬 New message from your coach",
        description: "Open Coach Messages below to read it.",
      });
    }
    prevUnreadRef.current = open ? -1 : unread;
  }, [unread, open]);

  // Mark as read and scroll when chat is opened
  useEffect(() => {
    if (open) {
      markRead.mutate();
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden transition-colors ${unread > 0 && !open ? "bg-primary/5 border-primary/40" : "bg-card border-card-border"}`}>
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
        <div className="relative w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
          <MessageCircle className={`w-4 h-4 ${unread > 0 ? "text-primary" : "text-primary"}`} />
          {unread > 0 && (
            <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] bg-destructive text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1">
              {unread}
            </span>
          )}
        </div>
        <div className="flex-1 text-left">
          <p className={`text-sm font-semibold ${unread > 0 ? "text-primary" : ""}`}>Coach Messages</p>
          <p className={`text-xs ${unread > 0 ? "text-primary/80 font-medium" : "text-muted-foreground"}`}>
            {unread > 0 ? `${unread} new message${unread > 1 ? "s" : ""} from your coach` : messages.length > 0 ? "Chat with your coach" : "No messages yet"}
          </p>
        </div>
        {open ? <ChevronDown className="w-4 h-4 text-muted-foreground" /> : <ChevronRight className={`w-4 h-4 ${unread > 0 ? "text-primary" : "text-muted-foreground"}`} />}
      </button>
      {open && (
        <div className="border-t border-border/30 flex flex-col" style={{ maxHeight: 320 }}>
          {/* Message thread */}
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2" style={{ maxHeight: 240 }}>
            {messagesQuery.isLoading ? (
              <div className="flex justify-center py-4"><Loader2 className="w-4 h-4 animate-spin text-primary" /></div>
            ) : messages.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-4">No messages yet. Say hi to your coach!</p>
            ) : (
              messages.map(msg => (
                <div key={msg.id} className={`flex ${msg.from_coach ? "justify-start" : "justify-end"}`}>
                  <div className={`max-w-[80%] px-3 py-2 rounded-2xl text-sm ${msg.from_coach ? "bg-muted text-foreground rounded-bl-sm" : "bg-primary text-primary-foreground rounded-br-sm"}`}>
                    <p>{msg.content}</p>
                    <p className={`text-[10px] mt-0.5 ${msg.from_coach ? "text-muted-foreground" : "text-primary-foreground/60"}`}>
          ...
          {/* Input */}
          <div className="flex gap-2 px-4 pb-4 pt-2 border-t border-border/30">
            <Input
              value={text}
            ...
            />
            <Button size="sm" disabled={!text.trim() || send.isPending} onClick={() => send.mutate(text.trim())} className="px-3 h-9">
  ...
  return (
    <div className="mobile-container overflow-y-auto scrollbar-none pb-24">
      ...
      <main className="px-6 pt-6 space-y-6">
        ...
        {/* Member: coach message thread */}
        {!isCoachView && user?.coachId && <CoachMessageCard />}
