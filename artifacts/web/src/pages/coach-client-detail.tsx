...
type Tab = "progress" | "messages" | "notes";
...
export default function CoachClientDetail() {
  ...
  const editNote = useMutation({
    mutationFn: ({ noteId, note }: { noteId: number; note: string }) =>
      customFetch(`/api/coach/clients/${clientId}/notes/${noteId}`, {
        method: "PUT",
    ...
    onSuccess: () => {
      ...
      setEditingNoteText("");
      qc.invalidateQueries({ queryKey: ["coach", "notes", clientId] });
    },
  ...
  return (
    <div className="mobile-container flex flex-col h-screen overflow-hidden bg-background">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1 text-xs font-medium hover:underline ${client.serviceTitle ? "text-primary" : "text-muted-foreground border border-dashed border-border rounded-lg px-2 py-0.5 hover:border-primary hover:text-primary"}`}
              >
                {client.serviceTitle
                  ? <>{client.serviceTitle}<span className="text-muted-foreground font-normal"> · {client.servicePrice} OMR</span></>
                  : "＋ Assign service"
                }
      ...
      <div className="px-5 pb-2 flex gap-1.5">
        {(["progress", "messages", "notes"] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "progress" ? "📈 Progress" : t === "messages" ? "💬 Messages" : "📝 Notes"}
          </button>
        ))}
      </div>
            ...
            {weightQuery.isLoading ? (
              ...
              <div className="bg-card border border-card-border rounded-2xl p-3">
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart
                    data={weightData.map(p => ({ ...p, date: new Date(p.date).toLocaleDateString("en", { month: "short", day: "numeric" }) }))}
                  ...
                  >
                    ...
                    <YAxis
                      ...
                      domain={([min, max]: [number, number]) => {
                        const padding = Math.max((max - min) * 0.5, 2);
          ...
          <div className="space-y-2 pt-1">
            ...
            {notesQuery.isLoading ? (
              ...
              (notesQuery.data ?? []).map(n => (
                <div key={n.id} className="bg-card border border-card-border rounded-xl px-3 py-2.5">
                  {editingNoteId === n.id ? (
                    /* ── Edit mode ── */
                    <div className="space-y-2">
                      <textarea
                        ...
                        rows={3}
                        autoFocus
                        className="w-full text-sm bg-background border border-primary/40 rounded-lg px-2.5 py-1.5 resize-none focus:outline-none focus:ring-1 focus:ring-primary"
                      />
                      <div className="flex gap-2 justify-end">
                        ...
                        <button
                          onClick={() => { if (editingNoteText.trim()) editNote.mutate({ noteId: n.id, note: editingNoteText.trim() }); }}
                          disabled={!editingNoteText.trim() || editNote.isPending}
                          className="flex items-center gap-1 text-xs text-primary font-medium px-2 py-1 rounded-lg bg-primary/10 hover:bg-primary/20 disabled:opacity-50"
                        >
                          {editNote.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Save
                        </button>
                      </div>
                    </div>
                  ) : (
                    ...
                    <div className="flex items-start gap-2">
                      <p className="text-sm flex-1 whitespace-pre-wrap">{n.note}</p>
